...
import { Badge, Card, Grid, IconButton } from "@mui/material";
...
function Form({ defNama, defNip, defTempat, defTanggal, defEmail, defTelp, defAlamat, defFoto }) {
    const [nip, setNip] = useState(defNip);
    const [nama, setNama] = useState(defNama);
    const [tempat, setTempat] = useState(defTempat);
    const [tanggal, setTanggal] = useState(defTanggal);
    ...
    const bodyFormData = {
        nip, nama, tmpt_lhr: tempat, tgl_lhr: tanggal, email, telp, alamat, foto
    }

    const onSubmit = () => {
        const token = localStorage.getItem("access_token");
        axios
            .put("/api/users", bodyFormData, { headers: { "x-auth-token": token } })
            .then((res) => {
                swal({
                    title: "Success",
                    text: res.data.msg,
